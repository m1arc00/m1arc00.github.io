 ...
 */
 ...
 //Added by Jesse Streb (brightcove)
 var bc_gMetaSet = false; //Before the video is played we should get the meta data.  We do this on load and the first time it is played since parent has WT meta tags added by JavaScript.
function dcsCQP(){
...
function dcsMetaCap(){
	...
	if (document.all){
	   try {
		elems=parent.document.all.tags("meta");
	   } catch (e) {}
	}
	else if (document.documentElement){
	   try {
		elems=parent.document.getElementsByTagName("meta");
	   } catch (e) {}
	}
...
function wtbaseInit() {
	if (dcsInit.enabled){
		...
		WT.mc_ev=dcsCQP();
		dcsFunc("dcsAdv");
		//Taking out so that we do not make the call onload.
		//dcsTag();
	}
}
